...
async def create_action_plan(instruction: str) -> list[dict]:
    ...
    if intent_info["intent"] == "product_search":
        ...
        if target_platform == "google_maps" or target_platform is None:
            ...
            context_parts.append("STRATEGY: Zomato requires a multi-step search process:")
            context_parts.append("1. Navigate to zomato.com/(city) - e.g., zomato.com/bangalore")
            context_parts.append("2. Click location dropdown (shows city)")
            context_parts.append("3. Type location within city (e.g., 'HSR')")
            context_parts.append("4. Select location suggestion (exact match case-insensitive, or first if not found)")
            context_parts.append("5. Wait 1-2 seconds after location selection")
            context_parts.append("6. Type food/dish in search bar")
            context_parts.append("7. Click first option in dish dropdown")
            context_parts.append("8. Extract restaurant details")
            context_parts.append("")
            context_parts.append("ZOMATO-SPECIFIC ACTION SEQUENCE:")
            context_parts.append('{"action": "navigate", "url": "https://www.zomato.com/bangalore"}')
            context_parts.append('{"action": "click", "selector": "div[class*=\\"sc-18n4g8v-0\\"], div[class*=\\"sc-fxmata\\"], div:has-text(\\"Bangalore\\")", "description": "Click location dropdown"}')
            context_parts.append('{"action": "type", "selector": "input[placeholder*=\\"location\\"], input[placeholder*=\\"area\\"]", "text": "LOCATION_NAME"}')
            context_parts.append('{"action": "click", "selector": "div[class*=\\"sc-\\"], [role=\\"option\\"]", "description": "Click location suggestion (exact match or first)"}')
            context_parts.append('{"action": "type", "selector": "input[placeholder*=\\"restaurant\\"], input[placeholder*=\\"cuisine\\"], input[placeholder*=\\"dish\\"]", "text": "FOOD_QUERY"}')
            context_parts.append('{"action": "click", "selector": "div[class*=\\"sc-\\"], [role=\\"option\\"], a[href*=\\"/bangalore/\\"]", "description": "Click first dish suggestion"}')
            context_parts.append('{"action": "wait_for", "selector": "div[class*=\\"sc-evWYkj\\"], div[class*=\\"sc-fYAFcb\\"], a[href*=\\"/bangalore/\\"]", "timeout": 10000}')
            context_parts.append('{"action": "extract", "schema": {...}, "limit": 10}')
            context_parts.append("")
            context_parts.append("IMPORTANT ZOMATO NOTES:")
            context_parts.append("- Navigate to city-specific URL: zomato.com/(city) - e.g., zomato.com/bangalore, zomato.com/mumbai")
            context_parts.append("- Location dropdown is usually on the left, shows current city")
            context_parts.append("- After clicking dropdown, type location within city (e.g., 'HSR', 'Indiranagar')")
            context_parts.append("- Location suggestions appear in dropdown - search for exact match (case-insensitive)")
            context_parts.append("- If exact match not found, select first suggestion from the list")
            context_parts.append("- Wait 1-2 seconds after location selection before typing food")
            context_parts.append("- Food search bar is usually on the right side of location dropdown")
            context_parts.append("- After typing food, click first option in dish dropdown (not just press Enter)")
            context_parts.append("- Zomato uses dynamic loading - wait_for should use container selectors")
            context_parts.append("")
            context_parts.append("EXTRACTION SCHEMA for Zomato:")
            context_parts.append('{')
            context_parts.append('  "name": "h4, h3, a[href*=\\"/bangalore/\\"], [class*=\\"sc-\\"]",')
            context_parts.append('  "rating": "[aria-label*=\\"star\\"], [class*=\\"rating\\"], [class*=\\"sc-\\"]",')
            context_parts.append('  "cuisine": "[class*=\\"cuisine\\"], text content matching cuisine pattern",')
            context_parts.append('  "location": "[class*=\\"location\\"], [class*=\\"area\\"]",')
            context_parts.append('  "price": "text matching ₹(\\d+) pattern",')
            context_parts.append('  "url": "a[href*=\\"/bangalore/\\"], a[href*=\\"zomato.com\\"]"')
            context_parts.append('}')
            context_parts.append("")
            context_parts.append("QUERY OPTIMIZATION for Zomato:")
            context_parts.append("- City: Extract from user instruction or default to 'bangalore'")
            context_parts.append("- Location query: Extract location within city (e.g., 'HSR', 'Indiranagar', 'Koramangala')")
            context_parts.append("  * Use location as-is, no city addition needed (city is in URL)")
            context_parts.append("  * Fix spelling errors if obvious")
            ...
            context_parts.append("- Example: User says 'best pizza places in HSR Bangalore'")
            context_parts.append("  * City: 'bangalore' (from URL: zomato.com/bangalore)")
            context_parts.append("  * Location: 'HSR'")
